...
void array_averaging(int parameter, int length, int* in_array, int* out_array) {
    ...
    for (int i = 0; i < length; i++) {
        calculation_interval(i, length, in_array, parameter, &start, &finish);
        sum = 0;
...
void accurate_search_extremes(int* x_array, int *length, int* y_array, int* result_array, int* out_length) {
    ...
    while(j < *length - 1) {
        if (((y_array[x_array[j]] < y_array[x_array[j - 1]]) && (y_array[x_array[j]] < y_array[x_array[j + 1]])) ||
            ((y_array[x_array[j]] > y_array[x_array[j - 1]]) && (y_array[x_array[j]] > y_array[x_array[j + 1]]))) {
            result_array[number_extrems] = x_array[j];
            number_extrems++;
        }
        j++;
    }
